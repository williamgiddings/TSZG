...
public class weapon_Base : MonoBehaviour {
    ...
    void IronSight()
    {
        StartCoroutine(_ironSight());
    }

    IEnumerator _ironSight (){
        if (CanIronsight)
        {
            if (!Sway.isIron)
            {
    ...
    void Shoot (){
	    ...
	    {	
            ...
            RaycastHit hit;
            Vector3 tempVec = Vector3.Slerp(Camera.main.transform.forward, Random.onUnitSphere, Spread);
            Ray ray = new Ray(Camera.main.transform.position, tempVec);
